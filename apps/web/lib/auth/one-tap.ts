 ...
 *
 * Kept out of `../auth-client` (which is `"use client"` and pulls in React) and
 * out of the component, so the values can be read and tested on their own.
 *
 * ## Why this file is careful about FedCM
 *
 * Google One Tap used to run as a Google-controlled iframe whose prompt the
 * page could inspect and steer. That flow is gone: Google's FedCM migration had
 * a transition period that **ended in August 2025**, after which the browser's
 * Federated Credential Management API is the only implementation and
 ...
 *     Services logs it as a deprecated setting and runs FedCM anyway, so an app
 *     that sets it is configured for a flow that no longer exists.
...
/**
 * Options for better-auth's `oneTapClient`, minus the client id (fetched at
 * runtime — see `hooks/use-auth-providers.ts`).
 *
 * `promptOptions.maxAttempts: 0` turns off the plugin's internal retry loop. It
 * is not "never retry": it is "the component decides", and
 * `components/auth/one-tap.tsx` re-prompts on a later navigation when the
 * outcome was not final. Leaving the plugin's loop on stacks a second FedCM
 * request onto a live one, which is the `InvalidStateError` above.
 *
