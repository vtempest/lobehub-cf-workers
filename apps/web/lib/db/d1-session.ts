 ...
 *
 * This module is shared architecture, ported from QwkSearch's
 * `lib/database/d1-session.ts` and deliberately kept behaviourally identical:
 * the same consistency rules, the same `D1_SESSION_MODE` rollback switch, and
 * the same bookmark header/cookie contract. Fix a bug in one, port it to the
 * other.
 *
